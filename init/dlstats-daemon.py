"""Init script. Daemonization of dlstats"""
import logging
import time
import daemon
import dlstats
...
class Job(object):
    ...
    def run(self):
        """Main loop for running update methods from each statistical
        provider."""
        while True:
            conf = dlstats.misc_func.get_config()
            jobs = []
            i = 10
            while i > 0:
